...
type ReqRow = {
  feature_id: number
  title: string
  status: 'Needs requirements' | 'Stories drafted'
  story_count: number
}

const FILTERS = ['All', 'Needs requirements', 'Stories drafted'] as const

function statusAccent(status: ReqRow['status']) {
  if (status === 'Needs requirements') return 'bg-amber'
  return 'bg-ok'
}

function statusChip(status: ReqRow['status']) {
  if (status === 'Needs requirements') return 'bg-amber-bg text-amber border-amber-bd'
...
export default function RequirementsBoardPage() {
  ...
  useEffect(() => {
    fetch('/api/requirements')
      .then((r) => r.json())
      .then((data: ReqRow[]) => setRows(Array.isArray(data) ? data : []))
      .catch(() => setRows([]))
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Requirements</h1>
        <p className="text-ink-3 text-[13.5px]">
          User stories and acceptance criteria for cases that have been admitted — before build
          starts.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-line rounded-xl3 px-6 py-12 text-center">
          <p className="text-ink-2 text-sm font-semibold mb-2">
            {filter === 'All'
              ? 'No admitted cases yet. Admit a case in Portfolio Review to start defining requirements.'
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {item.story_count > 0 && (
                      <span className="text-[10px] font-bold uppercase tracking-wide bg-surface-1 text-ink-2 border border-line rounded-full px-2 py-0.5">
                        {item.story_count} stor{item.story_count === 1 ? 'y' : 'ies'}
                      </span>
                    )}
                  </div>
                </div>
                <Link
                  href={`/requirements/${item.feature_id}`}
                  className="shrink-0 rounded-xl2 px-3.5 py-2 text-xs font-bold bg-bosch-red text-white"
                >
                  {item.status === 'Needs requirements' ? 'Draft stories →' : 'View stories →'}
                </Link>
              </div>
            </div>
          ))}
