...
type Row = {
  feature_id: number
  title: string
  status: 'Needs pipeline build' | 'Needs safety checks' | 'Safety cleared' | 'Safety blocked'
  has_build: boolean
}
...
function statusAccent(status: Row['status']) {
  if (status === 'Needs pipeline build' || status === 'Needs safety checks') return 'bg-amber'
  if (status === 'Safety blocked') return 'bg-bosch-red'
  return 'bg-ok'
}

function statusChip(status: Row['status']) {
  if (status === 'Needs pipeline build' || status === 'Needs safety checks') {
    return 'bg-amber-bg text-amber border-amber-bd'
  }
  if (status === 'Safety blocked') return 'bg-pink-bg text-pink border-pink-bd'
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Safety</h1>
        <p className="text-ink-3 text-[13.5px]">
          Security scan and cost check after the pipeline build — clear the gate before release.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-line rounded-xl3 px-6 py-12 text-center">
          <p className="text-ink-2 text-sm font-semibold mb-2">
            {filter === 'All'
              ? 'No admitted cases yet. Record a pipeline build first.'
