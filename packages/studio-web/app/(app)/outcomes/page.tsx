...
type Row = {
  feature_id: number
  title: string
  status: 'Needs live metrics' | 'Needs outcomes' | 'Outcomes judged'
}

const FILTERS = ['All', 'Needs live metrics', 'Needs outcomes', 'Outcomes judged'] as const

function statusAccent(status: Row['status']) {
  if (status === 'Outcomes judged') return 'bg-ok'
  return 'bg-amber'
}

function statusChip(status: Row['status']) {
  if (status === 'Outcomes judged') return 'bg-ok-bg text-ok border-ok-bd'
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Outcomes</h1>
        <p className="text-ink-3 text-[13.5px]">
          Judge whether live metrics delivered the outcome we set out to achieve.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-line rounded-xl3 px-6 py-12 text-center">
          <p className="text-ink-2 text-sm font-semibold mb-2">
            {filter === 'All'
              ? 'No admitted cases yet. Check rollout health first.'
          ...
          {visible.map((item) => (
            ...
            >
              ...
              <div className="flex-1 px-5 py-4 flex items-center gap-4">
                ...
                <Link
                  href={`/outcomes/${item.feature_id}`}
                  className="shrink-0 rounded-xl2 px-3.5 py-2 text-xs font-bold bg-bosch-red text-white"
                >
                  {item.status === 'Needs outcomes' ? 'Judge outcomes →' : 'View outcomes →'}
                </Link>
              </div>
            </div>
          ))}
