...
type Row = {
  feature_id: number
  title: string
  status: 'Needs outcomes first' | 'Needs impact assessment' | 'Impact assessed'
}

const FILTERS = ['All', 'Needs outcomes first', 'Needs impact assessment', 'Impact assessed'] as const

function statusAccent(status: Row['status']) {
  if (status === 'Impact assessed') return 'bg-ok'
  return 'bg-amber'
}

function statusChip(status: Row['status']) {
  if (status === 'Impact assessed') return 'bg-ok-bg text-ok border-ok-bd'
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Impact</h1>
        <p className="text-ink-3 text-[13.5px]">
          Assess how stakeholders felt after the outcomes landed.
        </p>
      </div>
          ...
          {visible.map((item) => (
            ...
            >
              ...
              <div className="flex-1 px-5 py-4 flex items-center gap-4">
                ...
                <Link
                  href={`/impact/${item.feature_id}`}
                  className="shrink-0 rounded-xl2 px-3.5 py-2 text-xs font-bold bg-bosch-red text-white"
                >
                  {item.status === 'Needs impact assessment' ? 'Assess impact →' : 'View impact →'}
                </Link>
              </div>
            </div>
          ))}
