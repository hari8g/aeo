...
type Row = {
  feature_id: number
  title: string
  status: 'Needs docs' | 'Docs drafted'
  docs_count: number
  note_count: number
}

const FILTERS = ['All', 'Needs docs', 'Docs drafted'] as const

function statusAccent(status: Row['status']) {
  return status === 'Needs docs' ? 'bg-amber' : 'bg-ok'
}

function statusChip(status: Row['status']) {
  return status === 'Needs docs'
    ? 'bg-amber-bg text-amber border-amber-bd'
    : 'bg-ok-bg text-ok border-ok-bd'
}

export default function DocsBoardPage() {
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Docs</h1>
        <p className="text-ink-3 text-[13.5px]">
          Product documentation outline and changelog for admitted cases after implementation.
        </p>
      </div>
