...
type Row = {
  feature_id: number
  title: string
  status: 'Needs quality check' | 'Quality passed' | 'Quality blocked'
  suite_count: number
  note_count: number
}

const FILTERS = ['All', 'Needs quality check', 'Quality passed', 'Quality blocked'] as const

function statusAccent(status: Row['status']) {
  if (status === 'Needs quality check') return 'bg-amber'
  if (status === 'Quality blocked') return 'bg-bosch-red'
  return 'bg-ok'
}

function statusChip(status: Row['status']) {
  if (status === 'Needs quality check') return 'bg-amber-bg text-amber border-amber-bd'
  if (status === 'Quality blocked') return 'bg-pink-bg text-pink border-pink-bd'
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Quality</h1>
        <p className="text-ink-3 text-[13.5px]">
          Run the quality gate after implementation — what passed, what failed, and whether
          we&apos;re blocked.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-line rounded-xl3 px-6 py-12 text-center">
          <p className="text-ink-2 text-sm font-semibold mb-2">
            {filter === 'All'
              ? 'No admitted cases yet. Record implementation first.'
