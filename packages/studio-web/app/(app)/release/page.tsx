...
function statusAccent(status: Row['status']) {
  if (status === 'Needs readiness check' || status === 'Awaiting release sign-off') return 'bg-amber'
  if (status === 'Not ready') return 'bg-bosch-red'
  return 'bg-ok'
}

function statusChip(status: Row['status']) {
  if (status === 'Needs readiness check' || status === 'Awaiting release sign-off') {
    return 'bg-amber-bg text-amber border-amber-bd'
  }
  if (status === 'Not ready') return 'bg-pink-bg text-pink border-pink-bd'
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Release</h1>
        <p className="text-ink-3 text-[13.5px]">
          Check readiness after safety clears, then approve the release before rollout.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-line rounded-xl3 px-6 py-12 text-center">
          <p className="text-ink-2 text-sm font-semibold mb-2">
            {filter === 'All'
              ? 'No admitted cases yet. Finish Safety first.'
          ...
          {visible.map((item) => (
            ...
            >
              ...
              <div className="flex-1 px-5 py-4 flex items-center gap-4">
                ...
                <Link
                  href={`/release/${item.feature_id}`}
                  className="shrink-0 rounded-xl2 px-3.5 py-2 text-xs font-bold bg-bosch-red text-white"
                >
                  {item.status === 'Needs readiness check'
                    ? 'Check readiness →'
                    : item.status === 'Awaiting release sign-off'
                      ? 'Approve →'
                      : 'View release →'}
                </Link>
              </div>
