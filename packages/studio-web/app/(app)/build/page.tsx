...
type Row = {
  feature_id: number
  title: string
  status: 'Needs implementation' | 'Implementation recorded'
  note_count: number
  context_count: number
}

const FILTERS = ['All', 'Needs implementation', 'Implementation recorded'] as const

function statusAccent(status: Row['status']) {
  return status === 'Needs implementation' ? 'bg-amber' : 'bg-ok'
}

function statusChip(status: Row['status']) {
  return status === 'Needs implementation'
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="text-[22px] font-extrabold tracking-tight mb-1">Implementation</h1>
        <p className="text-ink-3 text-[13.5px]">
          Record the changeset for admitted cases once architecture is proposed — files touched
          and what changed.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-line rounded-xl3 px-6 py-12 text-center">
          <p className="text-ink-2 text-sm font-semibold mb-2">
            {filter === 'All'
              ? 'No admitted cases yet. Finish Define (architecture) first.'
          ...
          {visible.map((item) => (
            ...
            >
              ...
              <div className="flex-1 px-5 py-4 flex items-center gap-4">
                ...
                <Link
                  href={`/build/${item.feature_id}`}
                  className="shrink-0 rounded-xl2 px-3.5 py-2 text-xs font-bold bg-bosch-red text-white"
                >
                  {item.status === 'Needs implementation' ? 'Record change →' : 'View change →'}
                </Link>
              </div>
            </div>
          ))}
